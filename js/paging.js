(function ($, window, document) {
    function Paging(el, options) {
        ...
        this.options = {
            ...
            slideSpeed: options.slideSpeed || 0,
    ...
    Paging.prototype = {
        constructor: Paging, init: function () {
            ...
            var that = this, ulDom = '', jumpDom = '', content = '', liWidth = 60, totalPages = that.options.totalPages,
                wrapLength = 0;
            totalPages > 5 ? wrapLength = 5 * liWidth : wrapLength = totalPages * liWidth;
            for (var i = 1; i <= that.options.totalPages; i++) {
                i != 1 ? ulDom += '<li class="pageNow">' + i + '</li>' : ulDom += '<li class="sel-page pageNow">' + i + '</li>';
            }
            ...
            var that = this, pageSelect = $('#pageSelect'), lis = pageSelect.children(), liWidth = lis[0].offsetWidth,
                totalPages = that.options.totalPages, pageIndex = that.options.pageNo, distance = 0,
                prePage = $('#prePage'), nextPage = $('#nextPage'), firstPage = $('#firstPage'),
                lastPage = $('#lastPage'), jumpBtn = $('#jumpBtn'), jumpText = $('#jumpText');
